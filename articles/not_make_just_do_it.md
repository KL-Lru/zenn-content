...
- ただのタスクを記述するだけなら`make`よりも`just`を使ってみよう

# `make` It!

`make`はプログラムやソフトウェアをビルドするコマンド/ツールです. [^1]
`Makefile`に記述された, ターゲットに関するスクリプトを実行することで, プログラムやソフトウェアのビルドやテスト, デプロイなどを行います.
...
この指定がない場合, 該当するターゲットと同一の名称のファイルまたはディレクトリが存在する場合, 該当のタスクが実行できなくなります.
...
`Makefile`では, コマンドライン引数を利用するためには, シェル変数として値を格納して実行する必要があります.
...
`Makefile`では, ターゲットに対するビルドコマンドの記述は Tab でインデントしなければなりません.
...
`Makefile`のようなレシピ(タスク)定義ファイル`justfile`を作成し, そこに記述されたレシピを実行できます.
...
ファイルやディレクトリがあったとしても, それを無視してレシピを実行します.
...
- 各タスクの簡易ドキュメント記述とヘルプ表示
- `.env`からの環境変数読み取り
- 各レシピへの環境変数の渡し込み
- 変数定義と展開
- 実行シェルの指定 (Shebang で指定可能な物)
    - Python なんかが指定可能

# まとめ

`Makefile`は「ビルドするタスク」を記述するためのものです.
何もビルドしないタスクを`make`で記述できますが, いくつか残念な点があります.
生成物を伴わないタスクであれば`just`のほうが`make`よりもタスクを簡素にかつ便利に記述できます.
逆にビルド処理が必要となるタスクの場合は依存関係解決の関係から, まだ`make`のほうが利点は多いでしょう.

あなたが書きたかったのは「仮想生成物のビルド処理を装ってタスクを実行する」`make`ではなく, 「ただタスクを実行する」`just`だったのではないでしょうか?
